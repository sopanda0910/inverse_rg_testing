...
Stages (sentinels in campaign_state/, prefix FU_):
  ...
  1. PYTEST    -- full u1_2d test suite (incl. the new ODE tests).
  ...
  5. EASY      -- 19 easy case (L=16, beta_f=4.44, 128 configs) where the
    ...
    .venv/Scripts/python.exe u1_2d/scripts/run_followup.py
"""
...
def run_stage(name, cmd, env_extra=None):
    ...
    env = {**os.environ, "U1_2D_TORCH_THREADS": "6",
...
def main():
    ...
    failures = []
    if not run_stage("PYTEST", [PY, "-m", "pytest", "u1_2d/tests", "-q"]):
        failures.append("PYTEST")

    if not run_stage("TAIL_ADAPT", [
            PY, "u1_2d/scripts/18_pq_hmc_tail.py",
    ...
    if not run_stage("SEED2", [
            PY, "u1_2d/scripts/06_generalization_study.py",
    ...
    if not run_stage("PROBES", [
            PY, "u1_2d/scripts/19_ode_reweighting.py",
