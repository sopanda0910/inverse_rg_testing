...
MAIN: list[tuple[str, str, str, list[tuple[str, str]]]] = [
    ("intro", "Introduction",
     "The claim: a diffusion-model configuration is a better HMC starting "
     "seed, and the regime where that matters is the one where HMC is "
    ...
    ("seedquality", "The result: seed quality",
     "The measurement and its yardstick; seven arms and one criterion; the "
     "tail never has to tunnel; whether the advantage survives volume.",
...
\title{Learned Prolongation for Lattice Gauge Theory:\\
       a diffusion-model configuration as an HMC starting seed}
\author{}
\date{}
...
\twocolumn[
  ...
  is that a diffusion-model configuration is a better HMC starting seed than
  any classical start, in the regime where standard HMC is topologically
  frozen. 2D compact U(1) is the primary study; 2D U(2) is the transfer
  demonstration.
  \end{abstract}
  \vspace{1.5em}
  \end{@twocolumnfalse}
