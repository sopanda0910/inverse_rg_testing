...
Designed for a detached overnight run on the Snapdragon laptop:
  ...
  * all output appends to OUT/run.log (the launcher redirects); stage markers
    ...
    .venv/Scripts/python.exe u1_2d/scripts/run_campaign.py [--smoke]
"""
...
def run_stage(name: str, cmd: list[str], threads: int = 6, critical: bool = False) -> bool:
    ...
    env = {**os.environ, "U1_2D_TORCH_THREADS": str(threads),
           "PYTHONUNBUFFERED": "1"}
    t0 = time.time()
...
def s2_case_ids() -> str:
    spec = importlib.util.spec_from_file_location(
        "study06", REPO / "u1_2d" / "scripts" / "06_generalization_study.py")
...
def main() -> None:
    ...
    if args.smoke:
        run_stage("SMOKE_STUDY", [PY, "u1_2d/scripts/06_generalization_study.py",
                                  "--smoke", *SAMPLER_FLAGS,
                                  "--checkpoint", "artifacts/diffusion/smoke/checkpoints/score_net.pt",
                                  "--out-dir", str(OUT / "smoke_study")], critical=True)
        run_stage("SMOKE_H2H", [PY, "u1_2d/scripts/14_diffusion_vs_instanton_hmc.py",
                                "--config", "u1_2d/configs/smoke.yaml",
                                "--checkpoint", "artifacts/diffusion/smoke/checkpoints/score_net.pt",
                                "--smoke", "--out-dir", str(OUT / "smoke_h2h")], critical=True)
        run_stage("SMOKE_ESS", [PY, "u1_2d/scripts/15_model_ess.py",
                                "--config", "u1_2d/configs/smoke.yaml",
                                "--checkpoint", "artifacts/diffusion/smoke/checkpoints/score_net.pt",
    ...
    run_stage("DATA", [PY, "u1_2d/scripts/01_generate_data.py",
                       "--config", CONFIG], critical=True)
    run_stage("TRAIN", [PY, "u1_2d/scripts/02_train.py",
                        "--config", CONFIG, "--resume"], critical=True)
    if not run_stage("LADDER", [PY, "u1_2d/scripts/03_run_ladder.py",
                                "--config", CONFIG]):
        failures.append("LADDER")
    if not run_stage("VALIDATE", [PY, "u1_2d/scripts/04_validate.py",
                                  "--config", CONFIG]):
        failures.append("VALIDATE")

    if not run_stage("STUDY_S1", [PY, "u1_2d/scripts/06_generalization_study.py",
    ...
    therm_cmd = [PY, "u1_2d/scripts/05_hmc_thermalization.py",
                 "--config", CONFIG,
    ...
    s2_cmd = [PY, "u1_2d/scripts/06_generalization_study.py",
    ...
    if not run_stage("HEADTOHEAD", [PY, "u1_2d/scripts/14_diffusion_vs_instanton_hmc.py",
                                    "--config", CONFIG]):
        failures.append("HEADTOHEAD")
    if not run_stage("ESS", [PY, "u1_2d/scripts/15_model_ess.py",
                             "--config", CONFIG,
                             "--cases", "16:14.1464", "16:55.0237",
                             "32:55.0237", "32:218.58",
                             "--n-configs", "64"]):
        failures.append("ESS")
    if not run_stage("AUTOCORR", [PY, "u1_2d/scripts/11_autocorrelation.py",
                                  "--dir", str(THERM_DIR)]):
        failures.append("AUTOCORR")
    if not run_stage("VERDICT", [PY, "u1_2d/scripts/12_campaign_verdict.py",
                                 "--study", str(GEN_DIR),
                                 "--out", str(OUT / "verdict")]):
