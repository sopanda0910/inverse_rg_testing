...
decomposition (blend on, sigmin 0.03) comes from script 28's samples instead.
    ...
    python u1_2d/scripts/27_matching_residual.py --sigma-min-coef 0.3 \
...
def run_case(model, schedule, case, args, action_type, device):
    ...
    # consumer below (snis_log_weights, coarse_only_features, the surrogate fits)
    # is CPU-side, matching conditional_ode_sample's CPU return. Without this the
    # case dies as soon as coarse meets fine on cuda.
    coarse = coarse.cpu()
...
def main() -> None:
    ...
    out_dir = Path(args.out or f"out/u1_2d/matching_residual/{action_type}")
