...
chain itself. All chains here run WITHOUT topological updates (plain HMC).
    ...
    python diffusion_v2/scripts/05_hmc_thermalization.py --config diffusion_v2/configs/demo.yaml
...
does thermalize, so 2 tau_int and burn-in are honest yardsticks:
    ...
    python diffusion_v2/scripts/05_hmc_thermalization.py --generalization
"""
...
from diffusion_v2.validate.report import validate_ensemble, freezing_diagnostics
from diffusion_v2.validate.stats import fit_exponential_relaxation, integrated_autocorrelation_time
...
def is_static(series: np.ndarray, discard: int) -> bool:
    """True when the equilibrated window has essentially no fluctuations, so
    tau_int measures nothing (the beta ~ 900 regime: the chain is a point)."""
    window = np.asarray(series[discard:], dtype=float)
    scale = max(abs(float(window.mean())), 1e-12)
    return float(window.std()) < 1e-7 * scale


def fitted_relaxation_taus(all_series: dict, targets: dict) -> dict:
    """Exponential-fit tau per start per observable (the cross-start comparison
    of characteristic relaxation times; TODO item 6)."""
    fits = {}
    for start, series in all_series.items():
        ...
        for name in targets:
            if name not in series:
                continue
            fit = fit_exponential_relaxation(series[name].mean(axis=1), targets[name])
            fits[start][name] = {
                "tau": fit.get("tau"),
                "tau_error": fit.get("tau_error"),
...
def run_rung(
    ...
    for name in targets:
        ...
        static[name] = is_static(hot_raw[name], discard)
    freezing = q_freezing(hot_raw["Q"], discard, f"hot-start HMC L={lattice_size} beta={beta:g}")
    relaxation_fits = fitted_relaxation_taus(all_series, targets)
    ...
    summary = {
        ...
        "tau_int_static": static,
        "interval_reliable": not any(static.get(n, False) for n in INTERVAL_OBS),
        "relaxation_fits": relaxation_fits,
        "hmc_interval_trajectories": slowest,
        "q_freezing": freezing,
        "wall_clock": {
            "gen_sample_seconds": meta.get("gen_seconds"),
            "n_seed_configs": int(seed_configs.shape[0]),
            "sec_per_traj_gen_batch": gen_spt,
            "sec_per_traj_baseline_batch": hot_spt,
        },
    }
...
def write_thermalization_report(
    ...
    for res in results:
        ...
        interval_str = (f"{interval:.1f}" if s.get("interval_reliable", True)
                        else f"{interval:.1f} (static chain -- not a mixing time)")
        ...
        lines.append(
            ...
            f"{interval_str} | {margin} | {fmt_t(slowest('hot start'))} / "
            f"{fmt_t(slowest('cold start'))} | {tau_q} |"
        )
    lines += ["", "## Wall-clock accounting", ""]
    lines += [
        "All timescales above are in HMC trajectories -- the honest *ergodicity* "
        "unit. This table converts to seconds on this machine so the economics "
        "are explicit. Batched chains produce n_chains configs per trajectory, "
        "so per-config costs divide by the chain count; the diffusion sampling "
        "cost amortizes over the whole generated batch.",
        "",
        "| case | seed: sample s/config | seed: t_therm s (batch) | HMC interval "
        "s/config | hot burn-in s (batch) | s/traj (batch) |",
        "|---|---|---|---|---|---|",
    ]
    for res in results:
        s = res["summary"]
        wc = s.get("wall_clock", {})
        spt = wc.get("sec_per_traj_baseline_batch") or s["hmc"].get("sec_per_traj_baseline_batch")
        spt_gen = wc.get("sec_per_traj_gen_batch") or s["hmc"].get("sec_per_traj_gen_batch")
        if not spt or not spt_gen:
            lines.append(f"| {s['label']} | n/a (summary predates wall-clock capture) | | | | |")
            continue
        n_base = max(int(s.get("n_baseline_chains", 1)), 1)
        gen_s = wc.get("gen_sample_seconds")
        n_seed = max(int(wc.get("n_seed_configs", s.get("n_gen_chains", 1))), 1)
        sample_per_cfg = f"{gen_s / n_seed:.1f}" if gen_s else "n/a (cached)"
        t_gen = max(s["t_therm"]["diffusion seed"][n] for n in wilson_obs)
        t_therm_s = f"{t_gen * spt_gen:.1f}" if math.isfinite(t_gen) else "never"
        interval_s = s["hmc_interval_trajectories"] * spt / n_base
        hot_burn = max(s["t_therm"]["hot start"][n] for n in wilson_obs)
        burn_s = f"{hot_burn * spt:.0f}" if math.isfinite(hot_burn) else "never"
        interval_note = "" if s.get("interval_reliable", True) else " (static)"
        lines.append(
            f"| {s['label']} | {sample_per_cfg} | {t_therm_s} | "
            f"{interval_s:.2f}{interval_note} | {burn_s} | {spt:.2f} |"
        )
    lines += [
        "",
        "## Fitted relaxation times across starts",
        "",
        "Exponential fits C + A exp(-t/tau) to the ensemble-mean plaquette and "
        "W(2x2) relaxation curves, per starting point (the cross-start comparison "
        "of characteristic times; a start already at its plateau fits no decay, "
        "which is the desired outcome for the diffusion seed).",
        "",
    ...
    def fmt_fit(fit):
        if fit is None:
            return "--"
        if fit.get("tau") is not None:
            err = fit.get("tau_error")
            return f"{fit['tau']:.1f} +- {err:.1f}" if err else f"{fit['tau']:.1f}"
        return fit.get("status", "--")

    for res in results:
        s = res["summary"]
        fits = s.get("relaxation_fits", {})
        for obs in ("plaquette", "wilson_2x2"):
            lines.append(
                f"| {s['label']} | {obs} | "
                f"{fmt_fit(fits.get('diffusion seed', {}).get(obs))} | "
                f"{fmt_fit(fits.get('hot start', {}).get(obs))} | "
                f"{fmt_fit(fits.get('cold start', {}).get(obs))} |"
            )
    lines += [
        "",
...
def run_generalization_scan(args, config: dict, device: str) -> None:
    ...
    for index, record in enumerate(cases):
        ...
        if raw_path.exists():
            ...
            print(f"    sampled in {gen_seconds:.0f}s", flush=True)
        ...
        meta = {"beta": beta_f, "lattice_size": fine_size, "gen_seconds": gen_seconds}
        if args.skip_cached:
...
def main() -> None:
    ...
    parser.add_argument("--config", default="diffusion_v2/configs/demo.yaml")
    ...
    parser.add_argument("--run-dir", default=None, dest="run_dir",
                        ...
                        "e.g. out/diffusion_v2/demo when the config says artifacts/diffusion/demo)")
    parser.add_argument("--generalization", nargs="?", const="out/diffusion_v2/demo/generalization",
                        default=None,
    ...
    parser.add_argument("--physics-blend-beta-min", type=float, default=0.0,
                        ...
                        help="generalization mode: gate the blend off below this beta")
