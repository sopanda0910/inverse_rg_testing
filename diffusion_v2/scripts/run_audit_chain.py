...
Stages (sentinels in out/diffusion_v2/audit_chain/state/, resumable):
  ...
  SUMMARY           audit_chain/report.md merging the key numbers
    ...
    .venv/Scripts/python.exe diffusion_v2/scripts/run_audit_chain.py
"""
...
def main() -> None:
    ...
    stage("PYTEST", [sys.executable, "-m", "pytest", "diffusion_v2/tests", "-q"], critical=True)
    stage("REBUILD_MATCHING", [
        sys.executable, str(SCRIPTS / "01_generate_data.py"),
        "--config", "diffusion_v2/configs/v2.yaml", "--rebuild-matching",
    ])
