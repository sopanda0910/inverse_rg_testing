...
$log = Join-Path $repo "out\diffusion_v2\ess_chain\scale_chain.log"
$wlog = Join-Path $repo "out\diffusion_v2\ess_chain\watcher.log"
...
$pidfile = Join-Path $repo "out\diffusion_v2\ess_chain\watcher.pid"
...
while ($true) {
    ...
    if (-not $alive) {
        ...
        if ($bad) {
            ...
            Start-Process -FilePath (Join-Path $repo "diffusion_v2\scripts\launch_scale_chain_safe.cmd") -WindowStyle Hidden
        } else {
            WLog "chain process gone (no critical events) -> relaunch #$relaunches (8 threads)"
            Start-Process -FilePath (Join-Path $repo "diffusion_v2\scripts\launch_scale_chain.cmd") -WindowStyle Hidden
        }
