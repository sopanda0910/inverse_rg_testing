...
\begin{table}[p]
  ...
  variance explained by coarse-only observables --- an upper bound on the
  matching-residual floor, since $c$-dependent model error lands there too.
  \textbf{Correction (2026-08-03):} the Villain arm as run used the
  \emph{Wilson}-matched coarse coupling ($\beta_c = 4.0$ instead of
  $14.1464/4 = 3.5366$), because \texttt{approx\_matched\_coarse\_beta} was
  called without its \texttt{action\_type} argument. The weights and
  certificate remain internally valid, but the row's premise --- ``$\beta/4$
  matching is exact, so this spread is pure model error'' --- did not hold as
  run; the mismatch contributes ${\sim}0.6$--1.5 nats against spreads of
  15--24 nats ($<1\%$ of variance), so the conclusion survives but the
  Villain numbers are an upper bound until the arm is rerun. Fixed in code.}
  ...
  \begin{tabular}{lrrr}
    ...
    Villain (exact matching \emph{as intended}) & 0.0287 (0.174) &
      0.0459 (0.031) & 0.0268 (0.048) \\
    \bottomrule
  \end{tabular}
\end{table}

Wilson $\le$ Villain everywhere, and Wilson's $R^2_c$ sits \emph{below}
Villain's model-error baseline: the coarse-action matching floor is
negligible, and the entire measured density gap is fine-side model error.
...
anneal away: max Wilson $|z| = 6.5$ at burn-in 1600 and 6.3 at 6400.
