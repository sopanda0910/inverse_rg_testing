...
sigma may be a per-configuration tensor so a whole training batch is noised
in one call.
"""
...
def noise_links(u0: torch.Tensor, sigma,
                generator: torch.Generator | None = None) -> tuple[torch.Tensor, torch.Tensor]:
    """Returns (u_t, omega). sigma is a scalar or a [B] tensor (one per config)."""
    if torch.is_tensor(sigma) and sigma.numel() > 1:
        s = (sigma**2).reshape(-1)
    else:
        s = float(sigma) ** 2 if not torch.is_tensor(sigma) else float(sigma.reshape(-1)[0]) ** 2
...
def proxy_score_target(omega: torch.Tensor, sigma) -> torch.Tensor:
    if torch.is_tensor(sigma) and sigma.numel() > 1:
        s2 = (sigma**2).reshape(-1, *([1] * (omega.dim() - 1)))
        return -omega / s2
    return -omega / (float(sigma) ** 2)


def exact_score_target(u_t: torch.Tensor, u_0: torch.Tensor, sigma) -> torch.Tensor:
